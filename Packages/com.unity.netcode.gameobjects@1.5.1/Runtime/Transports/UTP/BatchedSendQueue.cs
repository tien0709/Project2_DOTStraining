...
namespace Unity.Netcode.Transports.UTP
{
    ...
    internal struct BatchedSendQueue : IDisposable
    {
        ...
        private unsafe void WriteBytes(ref Unity.Networking.Transport.DataStreamWriter writer, byte* data, int length)
        {
        ...
        private void AppendDataAtTail(ArraySegment<byte> data)
        {
            unsafe
            {
                var writer = new Unity.Networking.Transport.DataStreamWriter((byte*)m_Data.GetUnsafePtr() + TailIndex, Capacity - TailIndex);
        ...
        public int FillWriterWithMessages(ref Unity.Networking.Transport.DataStreamWriter writer)
        {
            ...
            unsafe
            {
                var reader = new Unity.Networking.Transport.DataStreamReader(m_Data.AsArray());
        ...
        /// <param name="writer">The <see cref="DataStreamWriter"/> to write to.</param>
        /// <returns>How many bytes were written to the writer.</returns>
        public int FillWriterWithBytes(ref Unity.Networking.Transport.DataStreamWriter writer)
        {
            if (!IsCreated || Length == 0)
            {
                return 0;
